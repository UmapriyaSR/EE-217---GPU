...
void RestoreWeightsCUDA(NET* Net) {
    for (int l = 1; l < NUM_LAYERS; l++) {
        int numRows = Net->Layer[l]->Units;
        int numCols = Net->Layer[l-1]->Units + 1;
        
        REAL** dev_weight;
        REAL** dev_weightSave;

        // Allocate memory for the device pointers
        cudaMalloc(&dev_weight, numRows * sizeof(REAL*));
        cudaMalloc(&dev_weightSave, numRows * sizeof(REAL*));

        // Allocate memory for each row and copy the host pointers
        for (int i = 0; i < numRows; ++i) {
            cudaMalloc(&(dev_weight[i]), numCols * sizeof(REAL));
            cudaMalloc(&(dev_weightSave[i]), numCols * sizeof(REAL));
            cudaMemcpy(dev_weight[i], Net->Layer[l]->Weight[i], numCols * sizeof(REAL), cudaMemcpyHostToDevice);
            cudaMemcpy(dev_weightSave[i], Net->Layer[l]->WeightSave[i], numCols * sizeof(REAL), cudaMemcpyHostToDevice);
        }
        ...
        // Launch the kernel
        restoreWeightsKernel<<<numBlocks, threadsPerBlock>>>(dev_weight, dev_weightSave, numRows, numCols);

        // Copy the weights back to the host
        for (int i = 0; i < numRows; ++i) {
            cudaMemcpy(Net->Layer[l]->Weight[i], dev_weight[i], numCols * sizeof(REAL), cudaMemcpyDeviceToHost);
            cudaFree(dev_weight[i]);
            cudaFree(dev_weightSave[i]);
        }

        // Free the device pointers
        cudaFree(dev_weight);
        cudaFree(dev_weightSave);
    }
...
void PropagateNet(NET* net) {
    int numBlocks = ((NUM_LAYERS - 1) + BLOCK_SIZE - 1) / BLOCK_SIZE;
    propagateNetKernel<<<numBlocks, BLOCK_SIZE>>>(net);
 ...
 ******************************************************************************/
void InitializeRandomsCUDA(curandState **state, int num) {
    cudaMalloc(state, num * sizeof(curandState));
    initRandomsKernel<<<(num + 255) / 256, 256>>>(*state, 4711);
    // Error checking
}

void RandomEqualINTCUDA(curandState *state, int *output, int low, int high, int num) {
    generateRandomIntsKernel<<<(num + 255) / 256, 256>>>(state, output, low, high, num);
    // Error checking
}

void RandomEqualREALCUDA(curandState *state, REAL *output, REAL low, REAL high, int num) {
    generateRandomRealsKernel<<<(num + 255) / 256, 256>>>(state, output, low, high, num);
    // Error checking
}
